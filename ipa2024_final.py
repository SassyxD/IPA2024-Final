...
ACCESS_TOKEN = os.getenv('WEBEX_BOT_TOKEN')
...
# 4. Provide the URL to the Webex Teams messages API, and extract location from the received message.
    ...
    if message.startswith(f"/{STUDENT_ID}"):
        ...
        # extract the command
        command = message.split()[1] if len(message.split()) > 1 else ""
...
# 6. Complete the code to post the message to the Webex Teams room.
        ...
        if command == "showrun" and responseMessage == 'ok':
            filename = f"show_run_{STUDENT_ID}_router1.txt"
        ...
        print(f"Message sent successfully: {responseMessage}")
